<nav x-data="{ open: false, openNotif: false }" class="bg-gradient-to-r from-white via-white to-green-50 border-b-4 border-emerald-400 shadow-lg sticky top-0 z-40">
        ...
        <div class="flex justify-between h-20">
            <div class="flex">
                ...
                <div class="hidden space-x-6 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <x-nav-link :href="route('kehadiran')" :active="request()->routeIs('kehadiran.*')" class="inline-flex items-center gap-2 text-gray-600 hover:text-emerald-600 font-semibold transition duration-300 relative">
                        ...
                        @if(isset($perluAbsen) && $perluAbsen)
                            <span class="absolute top-5 -right-1 flex h-2 w-2">
                    ...
                    <x-nav-link :href="route('riwayat')" :active="request()->routeIs('riwayat.*')" class="inline-flex items-center gap-2 text-gray-600 hover:text-emerald-600 font-semibold transition duration-300 relative">
                        <svg class="w-5 h-5 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"></path></svg>
                        {{ __('Riwayat') }}

                        {{-- Logika baru: Cek jika ada notifikasi selain instruksi absen --}}
                        @php
                            $notifRiwayat = auth()->user()->unreadNotifications->filter(function($n) {
                                $pesan = strtolower($n->data['pesan'] ?? $n->data['message'] ?? '');
                                return !str_contains($pesan, 'absen') && !str_contains($pesan, 'check');
                            })->count();
                        @endphp
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6 gap-2">
                <div class="relative">
                    <button @click="openNotif = !openNotif" class="p-2 text-emerald-600 hover:bg-emerald-50 rounded-full transition relative focus:outline-none">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path></svg>
                        @if(auth()->user()->unreadNotifications->count() > 0)
                            <span class="absolute top-1 right-1 flex h-4 w-4">
                                <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-rose-400 opacity-75"></span>
                                <span class="relative inline-flex rounded-full h-4 w-4 bg-rose-500 text-[9px] text-white items-center justify-center font-black">{{ auth()->user()->unreadNotifications->count() }}</span>
                            </span>
                        @endif
                    </button>

                    <div x-show="openNotif" @click.away="openNotif = false" x-transition class="absolute right-0 mt-2 w-80 bg-white rounded-2xl shadow-xl border border-emerald-100 z-50 overflow-hidden" style="display: none;">
                        <div class="p-4 border-b border-emerald-50 bg-emerald-50/50"><h3 class="text-xs font-black text-emerald-800 uppercase tracking-widest">Notifikasi</h3></div>
                        <div class="max-h-64 overflow-y-auto">
                            @forelse(auth()->user()->unreadNotifications as $notification)
                                @php
                                    $pesan = $notification->data['pesan'] ?? ($notification->data['message'] ?? 'Ada pembaruan status');
                                    // Logika pengalihan URL berdasarkan isi pesan
                                    $isAbsen = str_contains(strtolower($pesan), 'absen') || str_contains(strtolower($pesan), 'check');
                                    $url = $isAbsen ? route('kehadiran') : route('riwayat');
                                @endphp
                                <a href="{{ $url }}" class="block p-4 border-b border-gray-50 hover:bg-emerald-50/30 transition">
                                    <p class="text-xs font-bold text-gray-800">{{ $pesan }}</p>
                                    <p class="text-[10px] text-gray-400 mt-1">{{ $notification->created_at->diffForHumans() }}</p>
                                </a>
                            @empty
                                <div class="p-6 text-center text-xs text-gray-400 font-medium">Tidak ada notifikasi baru</div>
                            @endforelse
                        </div>
                    </div>
                </div>
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-4 py-2 border-2 border-emerald-100 text-sm leading-4 font-bold rounded-full text-gray-700 bg-white/80 hover:bg-emerald-50 hover:border-emerald-300 transition duration-150 shadow-sm overflow-hidden relative">
            ...
            <div class="-me-2 flex items-center sm:hidden">
                <button @click="open = ! open" class="p-2 rounded-lg text-emerald-600 hover:bg-emerald-50 transition relative">
                    @if(auth()->user()->unreadNotifications->count() > 0)
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-white border-t border-emerald-50 shadow-inner">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('kehadiran')" :active="request()->routeIs('kehadiran.*')" class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <svg class="w-5 h-5 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                    <span class="font-bold">{{ __('Kehadiran') }}</span>
                </div>
                
                @if(isset($perluAbsen) && $perluAbsen)
                    <span class="bg-orange-500 text-white text-[10px] px-2 py-0.5 rounded-full font-black animate-pulse uppercase">Absen!</span>
                @endif
            </x-responsive-nav-link>
            ...
            <x-responsive-nav-link :href="route('riwayat')" :active="request()->routeIs('riwayat.*')" class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <svg class="w-5 h-5 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"></path></svg>
                    <span class="font-bold">{{ __('Riwayat') }}</span>
                </div>
                @if($notifRiwayat > 0)
                    <span class="bg-rose-500 text-white text-[10px] px-2 py-0.5 rounded-full font-black animate-pulse">BARU</span>
                @endif
            </x-responsive-nav-link>

            <x-responsive-nav-link :href="route('pengajuan')" :active="request()->routeIs('pengajuan.*')" class="flex items-center gap-3">
                <svg class="w-5 h-5 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                <span class="font-bold">{{ __('Pengajuan') }}</span>
            </x-responsive-nav-link>

            <x-responsive-nav-link :href="route('pengaturan')" :active="request()->routeIs('pengaturan')" class="flex items-center gap-3">
                <svg class="w-5 h-5 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.544.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
                <span class="font-bold">{{ __('Pengaturan') }}</span>
            </x-responsive-nav-link>
        </div>
        ...
        <div class="pt-4 pb-1 border-t border-emerald-100 bg-emerald-50/40">
            <div class="px-4 flex items-center">
                ...
                <div class="ms-3">
                    ...
                    <div class="font-medium text-xs text-emerald-700 italic">{{ Auth::user()->jabatan->nama_jabatan ?? 'Staff RSU' }}</div>
                </div>
            </div>
